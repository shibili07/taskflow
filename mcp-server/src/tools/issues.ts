...
export function registerIssueTools(server: McpServer) {
  server.registerTool(
    'taskflow_search_issues',
    {
      title: 'Search issues with JQL',
      description:
        'Search TaskFlow issues using a JQL-like query string (e.g. "project = ABC AND status = \\"In Progress\\""). Supports pagination.',
      inputSchema: {
